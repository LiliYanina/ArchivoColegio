...
class Estanterias extends Controller{
    ...
    {
        $data = $this->model->getEstanterias();
        for ($i=0; $i < count($data); $i++) { 
            if($data[$i]['estado'] == 1){
                $data[$i]['estado'] = '<span class="badge badge-success">Activo</span>';
                $data[$i]['acciones'] = '<div>
                <button class="btn btn-primary" type="button" onclick="btnEditarEstanteria(' .$data[$i]['id']. ');"><i class="fas fa-edit"></i></button>
                <button class="btn btn-danger" type="button" onclick="btnEliminarEstanteria(' .$data[$i]['id']. ');"><i class="fas fa-trash-alt"></i></button>  
                </div>';
            }else{
                $data[$i]['estado'] = '<span class="badge badge-danger">Inactivo</span>';
                $data[$i]['acciones'] = '<div>
                <button class="btn btn-success" type="button" onclick="btnReingresarEstanteria('.$data[$i]['id']. ');"><i class="fas fa-arrow-left"></i></button>
                </div>';
            }  
        }
    ...
    public function registrar()
    {
        ...
        if (empty($descripcion)){
           ...
           if($id == ""){
                    $data = $this->model->registrarEstanteria($descripcion); 
                    if ($data == "ok") {
                        $msg = "si";
                     }else if($data == "existe"){
                ...
                $data = $this->model->modificarEstanteria($descripcion, $id); 
                if ($data == "modificado") {
                    $msg = "modificado";
                }else{
    ...
    public function editar(int $id){
        //print_r($id);
        $data = $this->model->editarEstanteria($id);
        echo json_encode($data,JSON_UNESCAPED_UNICODE);
        die();
    }
    ...
    public function eliminar(int $id){
        //print_r($id);
        $data = $this->model->accionEstanteria(0, $id);
        if($data == 1){
            $msg = "ok";
        }else{
            $msg = "Error al eliminar el tipo la estanteria";
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    ...
    public function reingresar(int $id){
        //print_r($id);
        $data = $this->model->accionEstanteria(1, $id);
        if($data == 1){
            $msg = "ok";
        }else{
            $msg = "Error al reingresar el tipo la estanteria";
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    } 
